...
module.exports = {
   ...
   getuserAllProductslimit: () => {
      return new Promise(async (resolve, reject) => {
         let products = await db.get().collection(collection.PRODUCT_COLLECTION).find({ isActive: false }).limit(4).toArray()
         resolve(products)
      })
   },
   getuserAllProducts: () => {
      return new Promise(async (resolve, reject) => {
         let products = await db.get().collection(collection.PRODUCT_COLLECTION).find({ isActive: false }).toArray()
         resolve(products)
      })
   },
